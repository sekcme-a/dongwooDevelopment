...
const Building = () => {
  ...
  return(
    <div className={styles.main_container}>
      ...
      <Grid container spacing={2} rowSpacing={5} className={styles.list_container} ref={inViewRef} style={{position:"relative", marginTop:"60px"}}>
        ...
        {fetchedData.list?.map((item, index) => (
          <Grid item key={index} xs={12} md={6} lg={4}>
            ...
            <div style={{ width: '300px', height: '200px', border: '2px solid green',  borderRadius:"5px", overflow:"hidden", position:"relative"}}>
              <Image src={item.imgUrl} alt={item.title ?? "빌딩종합관리"} layout="fill" loading="lazy"/> 
            </div>
              <h4>{item.title}</h4>
              <p>{item.text}</p>
