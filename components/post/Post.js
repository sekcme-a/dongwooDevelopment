...
import AchievementPostHeader from "./AchievementPostHeader";
  ...
  if (!data || data?.condition !== "게제중")
    return (
      <div style={{ marginTop: "100px", marginBottom: "100px" }}>
        ...
        >
          <h6 style={{ width: "100vw", textAlign: "center" }}>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <div className={styles.written_info}>
            <p>
              <strong>게제일:</strong> {data?.publishedAt} /{" "}
              <strong>작성자:</strong> {data?.author}
            </p>
          </div>
          {type !== "achievements" ? (
            <h2>{data?.title}</h2>
          ) : (
            <AchievementPostHeader data={data} />
          )}
          ...
          {monitorWidth >= 900 && (
            <div className={styles.navbar_container}>
              ...
              <h6
                onClick={() =>
                  router.push(
                    `/${type === "advertisement" ? "job" : "article"}/${type}`
                  )
                }
              >
                목록으로
